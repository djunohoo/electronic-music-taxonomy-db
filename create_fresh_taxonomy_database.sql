-- =====================================================
-- CULTURAL INTELLIGENCE SYSTEM - STANDALONE DATABASE
-- =====================================================
-- Fresh Supabase database for taxonomy intelligence
...
CREATE TABLE duplicate_groups (
    ...
    primary_track_id BIGINT REFERENCES discovered_tracks(id),
...
CREATE TABLE track_analysis (
    id BIGSERIAL PRIMARY KEY,
    track_id BIGINT REFERENCES discovered_tracks(id),
...
CREATE TABLE processing_logs (
    id BIGSERIAL PRIMARY KEY,
    session_id BIGINT REFERENCES scan_sessions(id),
...
-- Discovery indexes
CREATE INDEX idx_discovered_tracks_hash ON discovered_tracks(file_hash);
CREATE INDEX idx_discovered_tracks_path ON discovered_tracks(file_path);
CREATE INDEX idx_discovered_tracks_status ON discovered_tracks(processing_status);

-- Analysis indexes
CREATE INDEX idx_track_analysis_track_id ON track_analysis(track_id);
CREATE INDEX idx_track_analysis_artist ON track_analysis(filename_artist);
CREATE INDEX idx_track_analysis_metadata_genre ON track_analysis(metadata_genre);

-- Profile indexes
CREATE INDEX idx_artist_profiles_name ON artist_profiles(normalized_name);
CREATE INDEX idx_label_profiles_name ON label_profiles(normalized_name);

-- Pattern indexes
CREATE INDEX idx_learned_patterns_type_value ON learned_patterns(pattern_type, pattern_value);
CREATE INDEX idx_learned_patterns_genre ON learned_patterns(genre);

-- Classification indexes
CREATE INDEX idx_track_classifications_track_id ON track_classifications(track_id);
CREATE INDEX idx_track_classifications_artist ON track_classifications(artist);
CREATE INDEX idx_track_classifications_genre ON track_classifications(primary_genre);

-- Session indexes
CREATE INDEX idx_scan_sessions_status ON scan_sessions(status);
CREATE INDEX idx_processing_logs_session ON processing_logs(session_id);

-- =====================================================
-- SAMPLE SEED DATA
-- =====================================================

-- Seed some initial patterns for bootstrap learning
INSERT INTO learned_patterns (pattern_type, pattern_value, genre, subgenre, confidence, sample_size, success_rate) VALUES
-- Filename patterns
...
-- Seed some known electronic music labels
INSERT INTO label_profiles (name, normalized_name, genre_specialization, confidence_score, sample_size) VALUES
...
INSERT INTO artist_profiles (name, normalized_name, primary_genres, confidence_score, sample_size) VALUES
...
SELECT 
    'CULTURAL INTELLIGENCE TAXONOMY DATABASE READY!' as status,
    COUNT(*) as tables_created
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN (
    'discovered_tracks', 'duplicate_groups', 'track_analysis', 
    'artist_profiles', 'label_profiles', 'learned_patterns',
    'track_classifications', 'scan_sessions', 'processing_logs'
);
